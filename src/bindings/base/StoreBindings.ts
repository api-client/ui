...
import { 
  ...
  Workspace, HttpProject, IAccessPatchInfo, IPatchInfo, IPatchRevision,
...
export abstract class StoreBindings extends PlatformBindings {
   ...
   */
  constructor(protected appId: string, protected appVersion: string) {
    super();
  }
  ...
  protected filePatchHandler(input: Event): void {
    const e = input as CustomEvent;
    e.preventDefault();
    e.detail.result = this.patchFile(e.detail.key, e.detail.id, e.detail.value, e.detail.media);
  }
  ...
  protected filePatchUserHandler(input: Event): void {
    const e = input as CustomEvent;
    e.preventDefault();
    e.detail.result = this.patchFileUsers(e.detail.key, e.detail.id, e.detail.value);
  }
   ...
   */
  patchFile(key: string, id: string, value: Patch.JsonPatch, media: false): Promise<IPatchRevision>;
   ...
   * @param value The patch to apply.
   */
  patchFile(key: string, id: string, value: Patch.JsonPatch, media: true): Promise<IPatchRevision>;
   ...
   */
  async patchFile(key: string, id: string, value: Patch.JsonPatch, media = false): Promise<IPatchRevision> {
    const { store } = this;
    if (!store) {
      throw new Error(`Environment is not set.`);
    }
   ...
   */
  async patchFileUsers(key: string, id: string, value: AccessOperation[]): Promise<void> {
    const { store } = this;
    if (!store) {
      throw new Error(`Environment is not set.`);
    }
